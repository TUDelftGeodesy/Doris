...
EPR_SProductId* epr_open_product(const char* product_file_path) {
    ...
    printf ("DEBUG: product: %s \n", product_id->id_string);
    /* Product identifier filter trick for ERS*/
    if (strncmp(EPR_ENVISAT_PRODUCT_SAR,  product_id->id_string, 3) == 0) // [MA]
      {
        product_id->id_string[0]='A'; // [MA] change 3 characters so that file is treated as ASAR
        product_id->id_string[1]='S';
        product_id->id_string[2]='A';
      }
    printf ("DEBUG: product: %s \n", product_id->id_string);
